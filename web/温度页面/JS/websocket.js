...
function onReceiveStr(blob){//字符串数据读取函数（当网页接收到来自服务器的数据时执行该函数，在函数websocket_Message中调用）
	...
	reader.onload = function(evt){
		...
		//获取的温度字符串中截取温度数值
		document.getElementById("Tem_1").innerHTML=str.substr(5,4);
	};
}
//---------------------------------------------------------------
...
//----------------------------------------------------------------
window.onload=function(){//监听网页加载完成的事件
	//当网页加载完成之后执行匿名函数（没有名字的函数），实现网页的自动联网功能
	/************************************************/
	//网络连接
	SocketConnect(2);
		TimeData();
		//delayed();
		setInterval("TimeData()",1000);  //毫秒为单位
	/************************************************/
}